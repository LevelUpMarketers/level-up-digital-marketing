...
class TotalTheme_WP_Import extends WP_Importer {
...
<?php endif; ?>
	...
	function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( ! is_wp_error( $id ) && $id > 0 ) {
				...
				do_action( 'totaltheme_demo_importer_insert_category', $id );
			} else {
	...
	function process_tags() {
		...
		foreach ( $this->tags as $tag ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				if ( ! empty( $id['term_taxonomy_id'] ) ) {
					do_action( 'totaltheme_demo_importer_insert_term', $id['term_taxonomy_id'] );
				}
			} else {
	...
	function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			if ( ! is_wp_error( $id ) ) {
				...
				/** Total Exclusive code */
				// Use correct ID's for nav menus so it works well with imported elements that have assigned menus
				if ( 'nav_menu' === $term['term_taxonomy']
					...
					$new_menu_id = (int) $term['term_id'];
					$old_menu_id = (int) $id['term_id'];
					
					$term_tax_exists = $wpdb->get_row( $wpdb->prepare(
					...
					if ( ! $term_tax_exists ) {
						...
						$wpdb->query( $wpdb->prepare(
							"UPDATE {$wpdb->terms} SET term_id = %d WHERE term_id = %d",
							$new_menu_id,
							$old_menu_id
						) );

						// Update the term_id in wp_term_taxonomy table
						$wpdb->query( $wpdb->prepare(
							"UPDATE {$wpdb->term_taxonomy} SET term_id = %d WHERE term_id = %d",
							$new_menu_id,
							$old_menu_id
						) );

						// Update the term_taxonomy_id in wp_term_relationships table
						$wpdb->query( $wpdb->prepare(
			...
			*/
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				...
				$postdata = array(
					...
					'post_content'   => apply_filters( 'totaltheme_demo_importer_post_content', $post['post_content'] ),
				...
				if ( 'attachment' == $postdata['post_type'] ) {
					...
					if ( $post_id ) {
						do_action( 'totaltheme_demo_importer_insert_post', $post_id );
					}
				}
				...
				foreach ( $post['terms'] as $term ) {
					...
					if ( ! $term_id ) {
						...
						if ( ! is_wp_error( $t ) ) {
							...
							if ( ! empty( $t['term_taxonomy_id'] ) ) {
								do_action( 'totaltheme_demo_importer_insert_term', $t['term_taxonomy_id'] );
							}
						} else {
			...
			if ( ! empty( $post['postmeta'] ) ) {
				foreach ( $post['postmeta'] as $meta ) {
					...
					if ( $key ) {
						...
						if ( ! $value ) {
							$value = maybe_unserialize( $meta['value'] );
							$value = apply_filters( 'totaltheme_demo_importer_meta_value', $value, wp_slash( $key ) );
						}
	...
	function process_menu_item( $item ) {
		...
		$args = array(
			...
			'menu-item-url'         => apply_filters( 'totaltheme_demo_importer_menu_item_url', $_menu_item_url ),
		...
		if ( $id && ! is_wp_error( $id ) ) {
			...
			do_action( 'totaltheme_demo_importer_insert_menu_item', (int) $id );
		}
	}
	...
	function process_attachment( $post, $url ) {
		...
		if ( $post_id ) {
			do_action( 'totaltheme_demo_importer_insert_attachment', $post_id );
		}
