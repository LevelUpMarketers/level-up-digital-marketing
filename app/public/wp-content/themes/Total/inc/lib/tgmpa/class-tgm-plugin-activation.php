	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			$this->strings = array(
				...
				'notice_can_install_recommended'  => _n_noop(
					'This theme includes the following optional plugin: %1$s.',
					'This theme includes the following optional plugins: %1$s.',
					'tgmpa'
				),
				/* translators: 1: plugin name(s). */
				...
				'notice_can_activate_recommended' => _n_noop(
					'The following optional bundled plugin is currently inactive: %1$s.',
					'The following optional bundled plugins are currently inactive: %1$s.',
					'tgmpa'
				),
		...
		protected function do_plugin_install() {
			...
			if ( ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' === $_GET['tgmpa-install'] ) || ( isset( $_GET['tgmpa-update'] ) && 'update-plugin' === $_GET['tgmpa-update'] ) ) {
				...
				/*
				// TOTAL FIXES - DON'T ACTIVATE PLUGINS DURING THE UPDATE PROCESS.
				// Only activate plugins if the config option is set to true and the plugin isn't
				// already active (upgrade).
				if ( 'update' == $install_type && $this->is_automatic && ! $this->is_plugin_active( $slug ) ) {
		...
		if ( isset( $_GET['page'] ) && $tgmpa_instance->menu === $_GET['page'] ) {
			...
			if ( ! class_exists( 'TGMPA_Bulk_Installer' ) ) {
				...
				class TGMPA_Bulk_Installer extends Plugin_Upgrader {
					...
					// 	TOTAL EDIT - DON'T ACTIVATE PLUGINS ON UPGRADE
					//	add_filter( 'upgrader_post_install', array( $this, 'auto_activate' ), 10 );
