 ...
 */
function wpex_has_registered_fonts(): bool {
 ...
 */
function wpex_get_registered_fonts(): array {
	$fonts = [];
	if ( is_callable( [ 'WPEX_Font_Manager', 'instance' ] )
		&& is_callable( [ WPEX_Font_Manager::instance(), 'get_registered_fonts' ] )
		&& $registered_fonts = WPEX_Font_Manager::instance()->get_registered_fonts()
	) {
 ...
 */
function wpex_get_font_family_stack( string $font_family, string $stack = '' ): string {
	$serif_fonts = [
		'Open Sans',
		'Lato',
		'Roboto',
	...
	if ( in_array( $font_family, $serif_fonts, true ) ) {
		$stack = 'sans-serif';
	}
	...
	$stack = (string) apply_filters( 'wpex_get_font_family_stack', $stack, $font_family );
...
if ( ! function_exists( 'wpex_add_custom_fonts' ) ) {
	function wpex_add_custom_fonts(): array {
		return (array) apply_filters( 'wpex_custom_fonts_array', [] );
	}
}
 ...
 */
function wpex_get_system_ui_font_stack(): string {
	$font_stack = 'system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, "Noto Sans", sans-serif, "Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol", "Noto Color Emoji"';
	return (string) apply_filters( 'wpex_get_system_ui_font_stack', $font_stack );
}

/**
 * List of standard fonts.
 */
function wpex_standard_fonts(): array {
	$standard_fonts = [
		'system-ui',
 ...
 */
function wpex_enqueue_google_font( $font = '' ): void {
 ...
 * Enqueues font.
 */
function wpex_enqueue_font( $font = '', $type = '', $args = [] ) {
	if ( class_exists( 'TotalTheme\Fonts\Font_Loader' ) ) {
		return (new TotalTheme\Fonts\Font_Loader( $font, $type, $args ))->font_url;
	}
}
 ...
 */
function wpex_get_font_type( $font = '' ): string {
	$type = 'google'; // this is the fallback font type in the theme
...
function wpex_render_custom_font_css( $font = '', $args = '' ) {
	...
	if ( $args ) {
		return totaltheme_call_non_static( 'Fonts\Render_Custom_Font_CSS', 'render', $font, $args );
	}
}
